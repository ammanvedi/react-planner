...
export default function PropertyLengthMeasure({ value, onUpdate, onValid, configs, sourceElement, internalState, state }, { catalog }) {
    ...
    let { hook, label } = configs, configRest = __rest(configs, ["hook", "label"]);
    ...
    return (<table className="PropertyLengthMeasure" style={PropertyStyle.tableStyle}>
      <tbody>
      <tr>
        ...
        <td style={secondTdStyle}>
          <table style={internalTableStyle}>
            ...
            <tr>
              <td>
                <FormNumberInput value={_length} onChange={event => update(event.target.value, _unit)} onValid={onValid} {...configRest}/>
              </td>
