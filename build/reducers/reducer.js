...
export default function appReducer(state, action) {
    ...
    if (ITEMS_ACTIONS[action.type])
        return ReactPlannerItemsReducer(...arguments);
    if (HOLE_ACTIONS[action.type])
        return ReactPlannerHolesReducer(...arguments);
    if (LINE_ACTIONS[action.type])
        return ReactPlannerLinesReducer(...arguments);
    if (AREA_ACTIONS[action.type])
        return ReactPlannerAreasReducer(...arguments);
    if (GROUP_ACTIONS[action.type])
        return ReactPlannerGroupsReducer(...arguments);
    if (SCENE_ACTIONS[action.type])
        return ReactPlannerSceneReducer(...arguments);
