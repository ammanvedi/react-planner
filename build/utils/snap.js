...
class LineSnap extends Record({
    ...
    nearestPoint(x, y) {
        return Object.assign(Object.assign({}, Geometry.closestPointFromLine(this.a, this.b, this.c, x, y)), { distance: Geometry.distancePointFromLine(this.a, this.b, this.c, x, y) });
    }
...
class LineSegmentSnap extends Record({
    ...
    nearestPoint(x, y) {
        return Object.assign(Object.assign({}, Geometry.closestPointFromLineSegment(this.x1, this.y1, this.x2, this.y2, x, y)), { distance: Geometry.distancePointFromLineSegment(this.x1, this.y1, this.x2, this.y2, x, y) });
    }
    isNear(x, y, distance) { return true; }
}
...
export function nearestSnap(snapElements, x, y, snapMask) {
    ...
    return snapElements
        ...
        .min(({ snap: { priority: p1 }, point: { distance: d1 } }, { snap: { priority: p2 }, point: { distance: d2 } }) => p1 === p2 ? (d1 < d2 ? -1 : 1) : (p1 > p2 ? -1 : 1));
}
...
export function addLineSnap(snapElements, a, b, c, radius, priority, related) {
    ...
    return snapElements.withMutations(snapElements => {
        let alreadyPresent = snapElements.some(lineSnap => lineSnap.type === 'line' &&
            a === lineSnap.a &&
            b === lineSnap.b &&
            c === lineSnap.c);
        if (alreadyPresent)
