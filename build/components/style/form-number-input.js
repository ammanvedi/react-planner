...
export default class FormNumberInput extends Component {
    ...
    render() {
        ...
        let numericInputStyle = Object.assign(Object.assign({}, STYLE_INPUT), style);
        if (this.state.focus)
        ...
        <input type="text" value={currValue} style={numericInputStyle} onChange={(evt) => {
            ...
            this.setState({ valid });
        }} onFocus={e => this.setState({ focus: true })} onBlur={e => this.setState({ focus: false })} onKeyDown={e => {
            var keyCode = e.keyCode || e.which;
            if ((keyCode == KEYBOARD_BUTTON_CODE.ENTER || keyCode == KEYBOARD_BUTTON_CODE.TAB) && different) {
                saveFn(e);
            }
        }} placeholder={placeholder}/>
        <div onClick={e => { if (different)
            saveFn(e); }} title={this.context.translator.t('Confirm')} style={Object.assign(Object.assign({}, confirmStyle), { visibility: different ? 'visible' : 'hidden', opacity: different ? '1' : '0' })}>
