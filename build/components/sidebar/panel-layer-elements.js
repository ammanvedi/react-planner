...
const elementSelectedStyle = Object.assign(Object.assign({}, elementStyle), { color: SharedStyle.SECONDARY_COLOR.main, borderColor: SharedStyle.SECONDARY_COLOR.main });
...
export default class PanelLayerElement extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (this.state.matchString !== nextState.matchString)
            return true;
        let oldElements = this.state.elements;
        let newElements = nextState.elements;
        if (oldElements.lines.hashCode() !== newElements.lines.hashCode() ||
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (this.props.layers.hashCode() === nextProps.layers.hashCode())
            return;
        let elements = {
            lines: layer.lines,
            holes: layer.holes,
    ...
    render() {
        if (!VISIBILITY_MODE[this.props.mode])
            return null;
        let layer = this.props.layers.get(this.props.selectedLayer);
        return (<Panel name={this.context.translator.t('Elements on layer {0}', layer.name)}>
        <div style={contentArea} onWheel={e => e.stopPropagation()}>
          ...
          {this.state.matchedElements.lines.count() ?
            <div>
                ...
                return (<div key={lineID} onClick={e => this.context.linesActions.selectLine(layer.id, line.id)} style={line.selected ? elementSelectedStyle : elementStyle}>
                        {line.name}
                      </div>);
            })}
              </div>
          ...
          {this.state.matchedElements.holes.count() ?
            <div>
                ...
                return (<div key={holeID} onClick={e => this.context.holesActions.selectHole(layer.id, hole.id)} style={hole.selected ? elementSelectedStyle : elementStyle}>
                        {hole.name}
                      </div>);
            })}
              </div>
          ...
          {this.state.matchedElements.items.count() ?
            <div>
                ...
                return (<div key={itemID} onClick={e => this.context.itemsActions.selectItem(layer.id, item.id)} style={item.selected ? elementSelectedStyle : elementStyle}>
                        {item.name}
                      </div>);
            })}
              </div>
