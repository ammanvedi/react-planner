...
export default class PanelGroupEditor extends Component {
    ...
    render() {
        ...
        return (<Panel name={this.context.translator.t('Group [{0}]', group.get('name'))} opened={true}>
          ...
          <table style={tableStyle}>
            <tbody>
              <tr>
                ...
                <td>
                  <FormTextInput value={group.get('name')} onChange={e => this.context.groupsActions.setGroupAttributes(this.props.groupID, new Map({ 'name': e.target.value }))} style={inputStyle}/>
                </td>
              </tr>
              <tr>
                <td style={firstTdStyle}>X</td>
                <td>
                  <FormNumberInput value={group.get('x')} onChange={e => this.context.groupsActions.groupTranslate(this.props.groupID, e.target.value, group.get('y'))} style={inputStyle} state={this.props.state} precision={2}/>
                </td>
              </tr>
              <tr>
                <td style={firstTdStyle}>Y</td>
                <td>
                  <FormNumberInput value={group.get('y')} onChange={e => this.context.groupsActions.groupTranslate(this.props.groupID, group.get('x'), e.target.value)} style={inputStyle} state={this.props.state} precision={2}/>
                </td>
              </tr>
              <tr>
                <td style={firstTdStyle}>{this.context.translator.t('Rotation')}</td>
                <td>
                  <FormNumberInput value={group.get('rotation')} onChange={e => this.context.groupsActions.groupRotate(this.props.groupID, e.target.value)} style={inputStyle} state={this.props.state} precision={2}/>
                </td>
              </tr>
            </tbody>
          </table>
            ...
            <div>
                ...
                return layerElements.entrySeq().map(([elementPrototype, ElementList]) => {
                    return ElementList.valueSeq().map(elementID => {
                        ...
                        return <tr key={elementID}>
                              <td style={iconColStyle} title={this.context.translator.t('Un-chain Element from Group')}>
                                <FaUnlink onClick={e => this.context.groupsActions.removeFromGroup(this.props.groupID, layerID, elementPrototype, elementID)} style={styleEditButton}/>
                              </td>
